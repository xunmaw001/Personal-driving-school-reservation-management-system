    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="jiaolianCollectionTypes">
                        <el-select v-model="ruleForm.jiaolianCollectionTypes" :disabled="ro.jiaolianCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in jiaolianCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="jiaolianCollectionValue">
                        <el-input v-model="ruleForm.jiaolianCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    jiaolianId: false,
                    yonghuId: false,
                    jiaolianCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    jiaolianId: '',
                    yonghuId: '',
                    jiaolianCollectionTypes: '',
                    insertTime: '',
                },
                jiaolianCollectionTypesOptions : [],
                jiaolianOptions : [],
                yonghuOptions : [],
                rules: {
                   jiaolianId: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiaolian_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.jiaolianCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `jiaolianCollection/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`jiaolianCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.jiaolianCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
